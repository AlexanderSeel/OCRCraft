...
- [x] Template- und Spielekatalog fachlich erweitern: zwei OCRFRA-Trainingsvorlagen und das Spiel `game-ocrfra-obstacle-route-relay` decken Altersstufen, OCR-Fähigkeiten, Zeitstruktur, Equipment, lokale Hindernisse, Progressionen und sichere Alternativen ab; Eigeninhalt und Provenienz sind in [`docs/template-game-curation.md`](./docs/template-game-curation.md) dokumentiert.
...
- [x] Browser-Regression-Gate für migrierte Katalogseiten ergänzt: Playwright erzeugt über `e2e/global-setup.ts` eine authentifizierte lokale Sitzung, verwendet eine isolierte DuckDB-Snapshot-Datenbank und prüft 34 Accessibility-, Dialog-, Layout-, Filter-, View-, Quick-Create- und mobile Regressionen.
