...
- [ ] Anatomisch geprüfte Bilder für alle Übungen ohne Bild erzeugen; `npm run exercise:images:codex-tasks` und der Download „Codex-Aufgaben herunterladen“ im Medienkatalog exportieren dafür den aktuellen DuckDB-Stand mit Exercise-ID, Seed-Key, Ersatzgrund, stabilem Dateinamen und dem vorhandenen OCRCraft-Bildprompt. Codex erzeugt/importiert nur prompt-fähige Aufgaben einzeln und lässt neue Medien bis Biomechanik-/Text-Match-Review auf `pending`.
