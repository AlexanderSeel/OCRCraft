...
- [ ] Codex-Bildimportaufgabe abschließen: die bereitgestellten Übungsbild-ZIP-Pakete über Codex anhand von `import-manifest.json`/`import-manifest.csv` in OCRCraft importieren; nur `exact`-Matches automatisch zuordnen, `near`/`variant`/`manual` fachlich prüfen, bestehende freigegebene Primärmedien nicht überschreiben, fehlende/unbrauchbare bzw. rechteblockierte Bilder gezielt ersetzen und anschließend die vollständige Abschlussroutine ausführen.
