...
export function QueueStatusIndicator() {
  ...
  return <div className="relative" ref={panelRef}>
    <button aria-expanded={open} aria-haspopup="dialog" aria-label="Hintergrundaufgaben anzeigen" className="flex min-h-9 items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 text-xs font-black shadow-sm" onClick={() => setOpen((value) => !value)} type="button">
    ...
    {open ? <div aria-label="Details der Hintergrundaufgaben" className="fixed right-4 top-16 z-[1000] w-[min(30rem,calc(100vw-2rem))] max-w-[calc(100vw-2rem)] rounded-xl border border-[var(--border)] bg-[var(--surface-elevated)] p-4 text-sm shadow-[var(--shadow-raised)]" role="dialog">
      <div className="flex items-start justify-between gap-4"><div><h2 className="font-black">Hintergrundaufgaben</h2><p className="mt-1 text-xs text-[var(--muted)]">{summary.running} läuft · {summary.queued} wartet · {summary.failed} Fehler</p></div><button aria-label="Details schließen" className="rounded-md border border-[var(--border)] px-2 py-1 text-xs font-bold" onClick={() => setOpen(false)} type="button">×</button></div>
