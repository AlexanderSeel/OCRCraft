...
export function Dialog({
  ...
  return createPortal(
    <div aria-labelledby={titleId} aria-modal="true" className="fixed inset-0 z-[2147483000] isolate flex items-center justify-center bg-black/[.52] p-4 shadow-[inset_0_0_120px_rgba(0,0,0,0.28)] backdrop-blur-[2px]" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} role="dialog">
      <div className={`relative z-[2147483001] max-h-[min(90vh,52rem)] w-full overflow-y-auto rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.42)] ${size === "wide" ? "max-w-5xl" : "max-w-2xl"}`} ref={panelRef}>
        ...
        <div className="dialog-content mt-4 min-w-0">{children}</div>
      </div>
    </div>,
    document.body,
  );
