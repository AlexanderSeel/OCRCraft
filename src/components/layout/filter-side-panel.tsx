...
export function FilterSidePanel({ title = "Filter", children }: { readonly title?: string; readonly children: React.ReactNode }) {
  ...
  return (
    <aside className={`relative h-fit min-w-0 rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-card)] transition-[width] lg:sticky lg:top-24 ${collapsed ? "lg:w-14" : "lg:w-[22rem] lg:max-w-full"}`}>
